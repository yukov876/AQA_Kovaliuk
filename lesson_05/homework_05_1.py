...
car_values = car_data.values()

year_filtr = {data for data in car_values if data[1]>=search_criteria[0]}

engine_volume_filtr = {data for data in year_filtr if data[2]>=search_criteria[1]}

price_filter = {data for data in engine_volume_filtr if data[-1]<=search_criteria[-1]}

sorted_car_values = (sorted(price_filter, key=lambda x: x[-1]))[0:5]

print(f"Перших п'ять авто у саиску, у яких рік випуску ≥ {search_criteria[0]}, об'єм двигуна ≥ {search_criteria[1]} та вартість ≤ {search_criteria[2]}:" )
for i in sorted_car_values:
  myKey = [key for key, value in car_data.items() if value == i]
  print(myKey)
