...
car_data_values3 = {key: value for key, value in car_data.items()
                  if value[1]>=search_criteria[0]
                  if value[2]>=search_criteria[1]
                  if value[-1]<=search_criteria[-1]}
print(f"Перших п'ять авто у списку, у яких рік випуску ≥ {search_criteria[0]}, об'єм двигуна ≥ {search_criteria[1]} та вартість ≤ {search_criteria[2]}:" )
print((sorted(car_data_values3.keys(), key=lambda x: x[-1]))[0:5])